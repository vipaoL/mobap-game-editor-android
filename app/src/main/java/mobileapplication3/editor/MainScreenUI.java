...
public class MainScreenUI extends Container {
    ...
    private void checkAutoSaveStorage() {
        ...
        try {
            elements = Platform.autoSaveRead();
        } catch (Exception ignored) { }
        if (elements != null && elements.length > 2) {
            Element[] finalElements = elements;
            showPopup(new AutoSaveRestorer(this, finalElements) {
    			public void onRestore() {
    				elementsBuffer.setElements(finalElements);
    				close();
    			};
    			
    			public void onDelete() {
                    Platform.deleteAutoSave();
    				close();
    			};
    		});
    	}
    ...
    private void saveToRMS() {
    	if (isAutoSaveEnabled && elementsBuffer != null) {
    		new Thread(new Runnable() {
				public void run() {
					try {
						Platform.autoSaveWrite(elementsBuffer);
					} catch (Exception ex) {
						ex.printStackTrace();
						Platform.showError(ex);
					}
